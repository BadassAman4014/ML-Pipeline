...
def plotHist(df, nameOfFeature):
    ...
    trace1 = go.Histogram(
        x=data_array,
        histnorm='',  # No normalization
        name=f'Histogram of {nameOfFeature}',
        autobinx=False,
        xbins=dict(
    ...
    layout = go.Layout(
        bargroupgap=0.3,
        title=f'The distribution of {nameOfFeature}',
        xaxis=dict(
            title=nameOfFeature,
            titlefont=dict(
    ...
    for i, offset in enumerate(clusters_separation):
        ...
        for i, (clf_name, clf) in enumerate(classifiers.items()):
            ...
            if clf_name == "Local Outlier Factor":
                ...
                scores_pred = clf.negative_outlier_factor_  # Use the negative_outlier_factor_ for LOF
            else:
                clf.fit(X)
            ...
            if clf_name == "Local Outlier Factor":
                # Plot points directly since scores_pred cannot be reshaped
                plt.scatter(new_df[feature1], new_df[feature2], c=scores_pred, cmap='coolwarm', s=20, edgecolor='k')
            else:
            ...
            plt.xlabel(feature1)
            plt.ylabel(feature2)
            plt.title(f"{i + 1}. {clf_name} (errors: {n_errors})")
...
param = {'C': 0.7678243129497218, 'penalty': 'l1', 'solver': 'saga'}
...
sns.heatmap(P.corr(), annot=True, fmt=".2f", cmap='coolwarm', square=True)
...
# Predict labels based on probability threshold
pp = [1 if p > 0.5 else 0 for p in p_sl[:, 1]]

# Calculate precision
precision = precision_score(y_test_sc, pp)
print("Super Learner Precision: %.3f" % precision)

# Calculate recall
recall = recall_score(y_test_sc, pp)
print("Super Learner Recall: %.3f" % recall)

# Calculate F1 score
f1 = f1_score(y_test_sc, pp)
print("Super Learner F1 Score: %.3f" % f1)

# Calculate and display the confusion matrix
conf_matrix = confusion_matrix(y_test_sc, pp)
print("\nConfusion Matrix:\n", conf_matrix)
